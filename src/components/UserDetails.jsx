...
const UserDetails = () => {
  ...
  const profileImageUrl = `https://192.168.1.188/hrwebapi/api/Files/GetProfileImage?userID=${UserID}`;
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        // ดึงข้อมูลผู้ใช้งาน
        const userResponse = await axios.get(`https://192.168.1.188/hrwebapi/api/Admin/Users/${UserID}`);
        setUser(userResponse.data);

        const userdata = userResponse.data

        // ดึงข้อมูลการศึกษา
        const educationResponse = await axios.get("https://192.168.1.188/hrwebapi/api/Admin/Educations");
        ...
        const workResponse = await axios.get("https://192.168.1.188/hrwebapi/api/Admin/WorkExperiences");
  ...
  const handleDelete = () => {
    if (modalExperienceID) {
      axios
        .delete(`https://192.168.1.188/hrwebapi/api/Admin/WorkExperiences/${modalExperienceID}`)
        .then(() => {
  ...
  const handleDelete1 = () => {
    if (modalEducationID) {
      axios
        .delete(`https://192.168.1.188/hrwebapi/api/Admin/Educations/${modalEducationID}`)
        .then(() => {
