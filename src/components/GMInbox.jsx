...
const GMInbox = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get("https://192.168.1.188/hrwebapi/api/LeaveRequest/gm/history");
      setHistory(res.data);
    } catch (error) {
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get("https://192.168.1.188/hrwebapi/api/LeaveRequest/gm/pending");
  ...
  const approve = async (id) => {
    ...
    try {
      ...
      await axios.post(
        `https://192.168.1.188/hrwebapi/api/LeaveRequest/gm/approve/${id}`,
        {
  ...
  const reject = async (id) => {
    ...
    try {
      setIsModalOpen(false);
      await axios.post(
        `https://192.168.1.188/hrwebapi/api/LeaveRequest/gm/reject/${id}`,
