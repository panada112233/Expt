...
const GMInbox = () => {
  ...
  const reject = async (id) => {
    ...
    try {
      ...
      // Ensure you're sending the rejectionReason as a Comment object
      await axios.post(
        `https://localhost:7039/api/LeaveRequest/gm/reject/${id}`,
        { Comment: rejectionReason }, // Send the rejection reason as "Comment" in the object
        {
          headers: {
            'Content-Type': 'application/json',
          },
                        ...
                        <tbody>
                          {currentRequest?.leaveStats &&
                            Object.entries(currentRequest.leaveStats).map(([key, stat]) => (
                              <tr key={key}>
                                <td className="capitalize font-FontNoto">{key}</td>
                                <td>{stat.used}</td>
                                <td>{stat.current}</td>
                                <td>{stat.total}</td>
                              </tr>
