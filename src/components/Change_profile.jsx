...
function ChangeProfilePicture() {
  ...
  useEffect(() => {
    const fetchProfileImage = async () => {
      try {
        const response = await axios.get(`https://192.168.1.188/hrwebapi/api/Files/GetProfileImage?userID=${userID}`);
        if (response.status === 200 ) {
          const fullImageUrl = `https://192.168.1.188/hrwebapi/api/Files/GetProfileImage?userID=${userID}`;
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setCurrentProfileImage(`https://192.168.1.188/hrwebapi/api${response.data.filePath}`);
