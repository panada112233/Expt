...
function ChangeProfilePicture() {
  ...
  useEffect(() => {
    const fetchProfileImage = async () => {
      try {
        const response = await axios.get(`http://192.168.1.188/hrwebapi/api/Files/GetProfileImage?userID=${userID}`);
        if (response.status === 200 ) {
          const fullImageUrl = `http://192.168.1.188/hrwebapi/api/Files/GetProfileImage?userID=${userID}`;
