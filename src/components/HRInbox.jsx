...
const HRInbox = () => {
  ...
  const fetchHistory = async () => {
    try {
      const res = await axios.get("https://192.168.1.188/hrwebapi/api/LeaveRequest/hr/history");
      setHistory(res.data);
    } catch (error) {
  ...
  const fetchRequests = async () => {
    setIsLoading(true);
    try {
      const res = await axios.get("https://192.168.1.188/hrwebapi/api/LeaveRequest/hr/pending");
  ...
  const showDetailModal = async (request) => {
    try {
      ...
      const res = await axios.get(`https://192.168.1.188/hrwebapi/api/LeaveRequest/stats/${userId}`);
  ...
  const approve = async () => {
    ...
    try {
      ...
      await axios.post(
        `https://192.168.1.188/hrwebapi/api/LeaveRequest/hr/approve/${currentRequest.id}`,
        {
  ...
  const sendBackToDocument = async () => {
    try {
      ...
      await axios.post("https://192.168.1.188/hrwebapi/api/Files/Create", formData);
